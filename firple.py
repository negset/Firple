...
SRC_FILES = {
    'Regular': [
        SRC_DIR + '/FiraCode-Regular.ttf',
        SRC_DIR + '/IBMPlexSansJP-Text.ttf',
        SRC_DIR + '/Firple-Italic.ttf'
    ],
    'Bold': [
        SRC_DIR + '/FiraCode-Bold.ttf',
        SRC_DIR + '/IBMPlexSansJP-Bold.ttf',
        SRC_DIR + '/Firple-BoldItalic.ttf'
    ]
}
...
def main():
    parser = argparse.ArgumentParser(
        description='Firple Generator v' + VERSION)
    parser.add_argument('-a', '--all', action='store_true',
                        help='generate all variants, weights and styles (default)')
    parser.add_argument('-s', '--single', choices=['slim', 'bold', 'italic'], nargs='*',
                        help='generate a single font file as specified; ignored if -a is set')
    parser.add_argument('--disable-nerd-fonts', dest='nerd', action='store_false',
    ...
    parser.add_argument('--show-fontforge-error', dest='suppress_error', action='store_false',
                        help='show fontforge error messages')
    args = parser.parse_args()

    print('Firple Generator v' + VERSION)

    ErrorSupressor.enable = args.suppress_error
    ...
    if args.all or args.single == None:
        # generate all variants, weights and styles
        generate(
            slim=False, bold=False, italic=False, nerd=args.nerd)      # Regular
        generate(
            slim=False, bold=False, italic=True, nerd=args.nerd)       # Italic
        generate(
            slim=False, bold=True, italic=False, nerd=args.nerd)       # Bold
        generate(
            slim=False, bold=True, italic=True, nerd=args.nerd)        # Bold Italic
        generate(
            slim=True, bold=False, italic=False, nerd=args.nerd)       # Slim Regular
        generate(
            slim=True, bold=False, italic=True, nerd=args.nerd)        # Slim Italic
        generate(
            slim=True, bold=True, italic=False, nerd=args.nerd)        # Slim Bold
        generate(
            slim=True, bold=True, italic=True, nerd=args.nerd)         # Slim Bold Italic
    else:
        # generate a single font file with specified styles
        generate(slim='slim' in args.single,
...
def generate(slim: bool, bold: bool, italic: bool, nerd: bool) -> bool:
    family = 'Firple Slim' if slim else 'Firple'
    ...
    if italic:
        ...
        if not os.path.exists(upright_tmp_path):
            ...
            ok = generate_upright(name, slim, weight,
                                  frcd_path, plex_path, upright_tmp_path)
            if not ok:
                return False
        # generate italic
        ok = generate_italic(name, slim, weight,
                             upright_tmp_path, ital_path, out_path)
        if not ok:
            return False
    else:
        # generate upright
        ok = generate_upright(name, slim, weight,
                              frcd_path, plex_path, out_path)
        if not ok:
            return False
...
def generate_upright(name: str, slim: bool, weight: str,
                     frcd_path: str, plex_path: str, out_path: str) -> bool:
...
def generate_italic(name: str, slim: bool, weight: str,
                    frpl_path: str, ital_path: str, out_path: str) -> bool:
    # check if src font files exist
    ...
    print('Generating temporary file...')
    frpl.fullname = name.replace('Firple', 'Tmp')
